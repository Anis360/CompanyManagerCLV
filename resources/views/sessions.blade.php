...
@section('content')
                    ...
                    <div class="panel-body">
                        ...
                        <table class="table table-bordered table-striped table-hover" id="table">
                            ...
                            @foreach ($sessions as $session)
                                <tr>
                                    <!--<td class="tdid">{{ $session->ID_Session }}</td>-->
                                    <td class="tdtheme">{{ $session->Theme_Session }}</td>
                                    <td class="tddb">{{ $session->Date_Debut }}</td>
                                    <td class="tddf">{{ $session->Date_Fin }}</td>
                                    <td class="tdnf1">{{ $session->Nom_Formateur1 }}</td>
                                    <td class="tdnf2">{{ $session->Nom_Formateur2 }}</td>
                                    <td class="tdsf">{{ $session->Nom_SF }}</td>
                                    <td class="tdet">{{ $session->Etat }}</td>
                                    <td class="tdcomm">{{ $session->Commentaire }}</td>
                                    @if(Auth::user()->role != 'Agent')
                        ...
                        <div class="form-group w-25">
                            ...
                            <div class="col-8">
                                <select id="frr1" name="frr1" class="form-control" required="required">
                                    <option>Sélectionner</option>
                                    @foreach ($fr as $f)
                                        <option value="{{ $f->Prenom_Nom_Formateur }}">
                                            {{ $f->Prenom_Nom_Formateur }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
    ...
    <script type="text/javascript">
        ...
        function getContent(){
            ...
            xhr.open('GET','{{route('SessionController.search')}}/?search=' + searchValue ,true);
...
<!--    <script type="text/javascript">
        $(document).ready(function(){
            ...
            $i = 1
            $('#editable').Tabledit({
                url:'{{ route("SessionController.action") }}',
                dataType:"json",
                columns:{
